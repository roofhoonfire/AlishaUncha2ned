...
public class BoundImageConductor : MonoBehaviour
{
    ...
    [SerializeField] private GameObject guessEditorRoot;   // 패널 루트 (SetActive on/off)
    [SerializeField] private TMP_InputField guessInput;    // 입력 필드
    [SerializeField] private Button guessSubmitButton;     // 제출 버튼 (Enter 제출 금지: 버튼만)
    ...
    // 아이콘 클릭 시 호출됨 — 네트워크 무관 (로컬 UI)
    public void BeginEditOpGuess(int index)
    {
        if (!isOp) return; // 내 바운드에서는 안 씀
        ...
        if (guessInput != null)
        {
            // Enter로 제출되지 않게 하려면 인스펙터에서 Line Type을 Multi Line Newline으로 설정
    ...
    public string InitBounds(IList<string> boundsFromData)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < _bounds.Count; i++)
                boundGuesses.Add($"상대의 {i+1}번째 바운드\n 상대의 행동에 집중해서 바운드를 예측하자");
    ...
    void BuildOpGridPreview(int count, IList<string> guesses)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            item.Set(spB0, desc); // 모두 b0 + 초기 guess
            _opIcons.Add(item);
