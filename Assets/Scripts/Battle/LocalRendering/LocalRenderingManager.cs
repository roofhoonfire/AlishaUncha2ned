...
public class LocalRenderingManager : MonoBehaviour
{
   ...
   public void ApplyImmediateUI(LocalRenderingData data)
    ...
    private IEnumerator Rendering_Norm_Action_Co(int actorNum, LocalRenderingData data1, LocalRenderingData data2, ActionData action, HookType h)
    {
        ...
        // [MOD] ¶ó¿ìÅÍ/DB Á¶È¸ ÁØºñ
        var router = CardAnimationRouter.Instance; // null °¡´É
        bool hasExactAnim = false;                 // ±âº»°ª: ¾øÀ½

        // [MOD] ¸ÞÀÎ ¾×¼Ç(ÀÌµ¿ ¾Æ´Ô)ÀÎ °æ¿ì¿¡¸¸ ¾Ö´Ï¸ÞÀÌ¼Ç Á¸Àç ¿©ºÎ¸¦ ¾ö°ÝÈ÷ °Ë»ç
        if (!isMoveAction && router != null && action != null)
        {
            hasExactAnim = router.HasExactEntry(action.cardcode, h); // Á¤È® ÀÏÄ¡¸¸ Çã¿ë
            if (!hasExactAnim)
            {
                Debug.Log($"[Rendering] Skip PlayCo: No exact anim for card='{action.cardcode}', hook='{h}'"); // [MOD] ·Î±×
            }
        }

        // ¸ÞÀÎ ¾×¼Ç Àü¿ë: ¼û±è Ã³¸®
        // [MOD] ±âÁ¸°ú ´Þ¸®, "Á¤È®È÷ µî·ÏµÇ¾î Àç»ýÇÒ ¶§¸¸" ¼û±ä´Ù.
        if (!isMoveAction && hasExactAnim)
        {
            if (CameraLovesAlisha.Instance != null &&
                ...
                actorGO != null)
            {
                CameraLovesAlisha.Instance.HideEmAll(actorGO); // ±âÁ¸ ÁÖ¼®/µ¿ÀÛ À¯Áö
            }
        }
        ...
        {
            // [MOD] ¸ÞÀÎ ¾×¼ÇÀÌ´õ¶óµµ, µî·ÏÀÌ ¾øÀ¸¸é PlayCo¸¦ "¾Æ¿¹ È£ÃâÇÏÁö ¾Ê´Â´Ù".
            if (hasExactAnim)
            {
            ...
            {
                // [MOD] µî·ÏÀÌ ¾øÀ» ¶§´Â ¾Æ¹« ¾Ö´Ïµµ Àç»ýÇÏÁö ¾Ê°í Åë°ú
                // ¼û±èµµ ÇÏÁö ¾Ê¾ÒÀ¸¹Ç·Î Unhide ºÒÇÊ¿ä
            }
        }
